...
def get_recurrent_model(learning_rate):
    ...
    concat_layers = Concatenate()([
        triangle_fig_model.layers[-2].output, face_model.layers[-2].output, hands_model.layers[-2].output])

    output = Dense(NUMBER_OF_CLASSES, activation='softmax')(concat_layers)

    rnn_model = keras.Model(
        [triangle_fig_model.input, face_model.input, hands_model.input], output)
    ...
    rnn_model.load_weights(
        '/home/alvaro/Desktop/multi-cue-sign-language/src/models/triangle_figure_face_hands_csl/').expect_partial()
...
def train_cnn_lstm_model(train_files, eval_files, epochs, batch_size, learning_rate, load_weights=False, tune_model=False, train_tuned_model=False):
    ...
    callbacks_list = [
        ModelCheckpoint('/home/alvaro/Desktop/multi-cue-sign-language/src/models/triangle_figure_face_hands_csl_v2/', monitor='val_accuracy',
